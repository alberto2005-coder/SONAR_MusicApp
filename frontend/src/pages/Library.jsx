...
import { Heart, Music, Plus, Play, Trash2, ChevronLeft, Download } from 'lucide-react';

const Library = () => {
    const navigate = useNavigate();
    const { user, token } = useContext(AuthContext);

    // playTrack gestiona la cola de reproducción
    ...
    // Sincronizar canciones si estamos en "Tus Me Gusta" y algo cambia
    useEffect(() => {
    ...
    return (
        <div className="w-full text-white p-8 pb-32">
            {selectedPlaylist ? (
                    ...
                    <div className="mt-10">
                        {playlistTracks.length > 0 ? (
                            <div className="space-y-1">
                                {playlistTracks.map((track, index) => (
                                    <div
                                        ...
                                        className={`group flex items-center justify-between p-3 rounded-md hover:bg-white/5 cursor-pointer transition-colors ${currentTrack?.id === track.id ? 'bg-white/10' : ''}`}
                                    >
                                        <div className="flex items-center gap-4">
                                            <span className={`w-4 text-center font-mono text-xs ${currentTrack?.id === track.id ? 'text-[#D4FF00]' : 'text-[#555]'}`}>
                                                {index + 1}
                                            </span>
                                            <img src={track.album?.cover_small} className="w-10 h-10 rounded shadow-md" alt="" />
                                            <div>
                                                <p className={`font-bold text-sm ${currentTrack?.id === track.id ? 'text-[#D4FF00]' : 'text-white'}`}>
                                                    {track.title}
                                                </p>
                                        ...
                                        <div className="flex items-center gap-4">
                                            ...
                                            <button
                                                ...
                                                className="text-[#555] hover:text-white transition opacity-0 group-hover:opacity-100"
                                            >
                                                <Download size={16} />
                                            </button>
                                            ...
                                            <button onClick={(e) => { e.stopPropagation(); toggleFavorite(track); }}>
                                                <Heart
                                                    size={16}
                                                    fill={isFavorite(track.id) ? "#D4FF00" : "none"}
                                                    className={isFavorite(track.id) ? "text-[#D4FF00]" : "text-[#555] hover:text-white"}
                                                />
                                            </button>

                                            <div className="text-xs text-[#555] w-10 text-right font-mono">
                            ...
                            <div className="flex flex-col items-center justify-center py-20 border border-dashed border-[#222] rounded-xl bg-[#0a0a0a]">
                                ...
                                <button onClick={() => navigate('/search')} className="bg-white text-black px-8 py-3 rounded-full font-bold hover:bg-[#D4FF00] transition-all hover:scale-105 shadow-xl">
                                    Buscar canciones
                                </button>
                            </div>
                        )}
                ...
                // --- VISTA REJILLA (GRILLA) DE BIBLIOTECA ---
                <>
                    ...
                    {showCreate && (
                        <form onSubmit={(e) => { e.preventDefault(); if (newPlaylistName.trim()) { createPlaylist(newPlaylistName); setNewPlaylistName(''); setShowCreate(false); } }} className="mb-8 bg-[#121212] p-4 rounded-lg flex gap-4 border border-[#222]">
                            <input autoFocus className="bg-[#1a1a1a] flex-1 p-2 rounded outline-none border border-transparent focus:border-[#555] text-sm" placeholder="Nombre de la nueva lista..." value={newPlaylistName} onChange={(e) => setNewPlaylistName(e.target.value)} />
                        ...
                        {playlists.map((playlist) => (
                            <div key={playlist.id} className="group bg-[#121212] p-4 rounded-md hover:bg-[#181818] transition-all relative cursor-pointer" onClick={() => openPlaylist(playlist)}>
                                <div className="aspect-square bg-[#1a1a1a] mb-4 flex items-center justify-center rounded-lg relative overflow-hidden shadow-inner">
                                    ...
                                    <button onClick={(e) => { e.stopPropagation(); deletePlaylist(playlist.id); }} className="absolute top-2 right-2 p-2 bg-black/60 text-red-500 rounded-full opacity-0 group-hover:opacity-100 hover:bg-red-500 hover:text-white transition-all z-10">
                                        <Trash2 size={16} />
                                    </button>
                                </div>
